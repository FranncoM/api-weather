...
const getForecast = async (lat, lon) => {
  const response = await fetch(
    `${baseUrl}onecall?lat=${lat}&lon=${lon}&exclude=minutely,hourly&units=metric&appid=${apiKey}`
  );
...
const getDirectGeoCoding = async (city) => {
  ...
  const limit = 1;
  const response = await fetch(
    `${geoUrl}direct?q=${cityEncoded}&limit=${limit}&appid=${apiKey}`
  );
  const data = await response.json();
...
const getReverseGeoCoding = async (lat, lon) => {
  const limit = 1;
  const response = await fetch(
    `${geoUrl}reverse?lat=${lat}&lon=${lon}&limit=${limit}&appid=${apiKey}`
  );
  const data = await response.json();
